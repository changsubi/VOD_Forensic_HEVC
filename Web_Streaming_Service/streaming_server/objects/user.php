...
";
    ...
    function save($updateUserGroups = false) {
        ...
        $this->phone = $global['mysqli']->real_escape_string($this->phone);
        ...
        if (!empty($this->id)) {
            $formats = "sssssiii";
            $values = array($this->user, $this->password, $this->email, $this->name, $this->phone, $this->isAdmin, $this->canStream, $this->canUpload);
            $sql = "UPDATE users SET user = ?, password = ?, "
                    . "email = ?, name = ?, phone = ?, isAdmin = ?,"
                    . "canStream = ?,canUpload = ?,";
            ...
            $formats = "sssssiiissssss";
            $values = array($this->user, $this->password, $this->email, $this->name, $this->phone, $this->isAdmin, $this->canStream, $this->canUpload,
                $this->status, $this->photoURL, $this->recoverPass, $this->channelName, $this->analyticsCode, $this->externalOptions);
            $sql = "INSERT INTO users (user, password, email, name, phone, isAdmin, canStream, canUpload, canViewChart, status,photoURL,recoverPass, created, modified, channelName, analyticsCode, externalOptions) "
                    . " VALUES (?,?,?,?,?,?,?,?, false, "
                    . "?,?,?, now(), now(),?,?,?)";
        }
    ...
    static function createUserIfNotExists($user, $pass, $name, $phone, $email, $photoURL, $isAdmin = false, $emailVerified = false) {
