                    ...
                    if (CustomizeUser::canDownloadVideosFromVideo($video['id'])) {
                        ...
                        foreach ($files as $key => $theLink) {
                            ...
                            if (strpos($theLink['url'], '?') === false) {
                                $theLink['url'] .= "_out.hevc?download=1&title=" . urlencode($video['title'] . "_{$key}_.hevc");
                            }
    ...
    <div class="row bgWhite list-group-item menusDiv" id="downloadDiv">
        <div class="tabbable-panel">
            ...
            foreach ($filesToDownload as $theLink) {
                    $theLink['name'] = 'HEVC';
                    ?>
                    <a onclick="insertchk();" class="list-group-item list-group-item-action" style="cursor: pointer"  download>
                        <i class="fas fa-download"></i> <?php echo $theLink['name']; ?>

		    </a>
                    <script type="text/javascript">
		    function insertchk(){
			    var video_cmd = '<?= "/var/www/html/WKstream/videos/".$video['filename']."_Low.mp4"?>';
                var video_name = '<?= $video['filename']."_Low"?>';
                var user_id = '<?= $_SESSION['user']['user']?>';
                var user_name = '<?= $_SESSION['user']['name']?>';
                var user_email = '<?= $_SESSION['user']['email']?>';
			    var user_phone = '<?= $_SESSION['user']['phone']?>';
			    var identification_char = SHA256(makeid(10));
			    var download_chk = false;

			    var interval = null;
			    $('#loading > div').css({
                    "top" : (($(window).height()-$('#loading > div').outerHeight())/2+$(window).scrollTop()-40)+"px",
                    "left" : (($(window).width()-$('#loading > div').outerWidth())/2+$(window).scrollLeft())+"px"
                });
                $('#loading > p').css({
                    "top" : (($(window).height()-$('#loading > div').outerHeight())/2+$(window).scrollTop()-40+45)+"px",
                    "left" : (($(window).width()-$('#loading > div').outerWidth())/2+$(window).scrollLeft())+"px"
                });
                $('#loading').show();
                ...
                interval = setInterval(function() {
                    if(download_chk == false){
                        jQuery.ajax({
                            type: "POST",
                            url: "/WKstream/view/chk_percent.php?identification_char=" + identification_char,
                            dataType:"json",
                            success: function (response) {
                                $('#loading > p').text(response.percent+'%');
                            }
                        });
                    }else if(download_chk == true){
                        download_chk = false;
                ...
                jQuery.ajax({
                    type: "POST",
                    url: "/WKstream/view/forensic.php?invideo=" + video_cmd + "&hevcvideo=" + video_name + "&user_id=" + user_id + "&user_name=" + user_name + "&user_email=" + user_email + "&user_phone=" + user_phone + "&identification_char=" + identification_char,
                    //dataType:"json",
                    success: function () {
                        ...
                        download_chk = true;
                        $('#loading > p').text('0%');
                        location.href = "<?php echo $theLink['url']; ?>";
                        return true;
                    },
                    error: function (err) {
                        //console.error("error");
